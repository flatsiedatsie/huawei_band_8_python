...
async def main():
    ...
    for device in devices:
        ...
        #print("device_dict: " + json.dumps(device_dict,indent=4))
        print("device_dict: " + str(device_dict.keys()))
    ...
    for device in devices:
        try:
            ...
            if device.name == None:
                print("skip - None")
                continue
            if not 'HUAWEI Band 8' in device.name:
                print("skip - not huawei: ", device.name)
                continue
                
            print("\n\nNAME: ", device.name)
            
                
            temp_device = await BleakScanner.find_device_by_address(device.address, timeout=5)
