...
def request_authentication(auth_version: AuthVersion, client_nonce: bytes, server_nonce: bytes) -> Packet:
    print("device_config.py: in request_authentication")
    
    #See: https://codeberg.org/Freeyourgadget/Gadgetbridge/src/branch/master/app/src/main/java/nodomain/freeyourgadget/gadgetbridge/devices/huawei/packets/DeviceConfig.java
    return Packet(
        service_id=DeviceConfig.id,
        ...
        command=Command(
            tlvs=[
                ...
                #TLV(tag=DeviceConfig.Auth.Tags.deviceUUID, value=(self['config']["device_uuid"]),
                #TLV(tag=DeviceConfig.Auth.Tags.Tag3, value=b"\x01"),
                #TLV(tag=DeviceConfig.Auth.Tags.Tag4, value=b"\x00"),
            ],
        ),
    )
...
def request_bond_params(client_serial: str, client_mac: str) -> Packet:
    print("device_config.py: in request_bond_params")
    return Packet(
        service_id=DeviceConfig.id,
...
def get_supported_commands(auth_version: AuthVersion, client_serial: str, device_mac: str, key: bytes, iv: bytes) -> Packet:
    return Packet(
        service_id=DeviceConfig.id, # 1
        command_id=DeviceConfig.SupportedCommands.id, # 61 ?? 55?
        #service_id=55, # 1
        #command_id=127, # 61 ?? 55?
        command=Command(
            tlvs=[
                #TLV(tag=DeviceConfig.ReplyOK.Tags.setStatus, value=b"\x20"),
                TLV(tag=DeviceConfig.SupportedCommands.Tags.GetStatus), # , value=b"\x000186A0"
            ],
        ),
    )
    

def reply_ok(auth_version: AuthVersion, client_serial: str, device_mac: str, key: bytes, iv: bytes) -> Packet:
    return Packet(
        #service_id=DeviceConfig.id, # 1
